...
const APPHOST = 'http://192.168.20.114:8080/etcApi/'; // 接口地址
...
$(document).ready(function(){
	...
	$('.Assets_ul_tab p').click(function(){
		...
		$('.Assets_ul_table').eq($(this).index()).addClass('active').siblings('.Assets_ul_table').removeClass('active');
		$('.Assets_ul_tab_info').eq($(this).index()).addClass('active').siblings('.Assets_ul_tab_info').removeClass('active');
	})
	...
	$('.Assets_list_bottom').click(function(){
		...
		$(this).addClass('on');
		$(this).parent('li').siblings('li').children('.Assets_list_bottom').children('img').attr('src','images/choose_no.png');
		$(this).parent('li').siblings('li').children('.Assets_list_bottom').removeClass('on');
	})
})
